# 6bar6_3 Link Dimensions

Source: `6bar6_3.motiongen` with `settings.json` (`autoscale.target_delta_y = 250 mm`, applied scale `0.887117×`). All lengths are centerline distances between joint axes, in millimeters.

## Link dimensions

| Link | Type | Role | Edge | Length (mm) |
|---|---|---|---|---|
| **L1** | binary | Crank (motor at B) | A–B | 38.83 |
| **L2** | ternary | Ground frame | B–C | 58.68 |
| | | | C–D | 32.37 |
| | | | B–D | 89.00 |
| **L3** | ternary | Coupler (E = output) | E–F | 182.27 |
| | | | F–G | 27.97 |
| | | | E–G | 166.12 |
| **L4** | binary | Rocker (loop 2) | D–G | 162.09 |
| **L5** | ternary | Connects both loops | C–F | 153.16 |
| | | | F–H | 74.56 |
| | | | C–H | 78.64 |
| **L6** | binary | Rocker (loop 1) | A–H | 69.16 |
...
- Regenerate this table by running `python sixbar_sim.py 6bar6_3.motiongen --no-gui` after any geometry or autoscale change.
